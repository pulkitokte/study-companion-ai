...
import { initNetworkManager, getNetworkState, onNetwork }      from '../lib/networkManager.js'
import { startSyncScheduler, getSyncSchedulerStatus, runSyncCycle, onSyncEvent } from '../lib/syncScheduler.js'
import { getQueueStats }  from '../lib/offlineQueue.js'
import { getCacheStats }  from '../lib/cacheManager.js'
// Merged — was two separate imports from errorLogger.js
import { getLogStats, installGlobalErrorHandler } from '../lib/errorLogger.js'
...
export function SystemProvider({ children }) {
  ...
  }, [cacheStats])   // cacheStats changing is a reliable proxy for storage mutation
  ...
  const systemHealth = useMemo(() => {
    const checks = [
      ...
      { id:'queue',    ok: (syncStatus.queue?.dead ?? 0) < 5, label:'Sync Queue' },
  ...
  }, [networkState, storageSize, logStats, syncStatus])
  ...
  const value = useMemo(() => ({
    ...
    networkState, syncStatus, cacheStats, logStats, lastEvent,
    storageSize, systemHealth, triggerSync, refreshStats,
  ])
