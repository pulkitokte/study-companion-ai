...
import { getCategoryBreakdown, getWeeklyTrend } from '../lib/analyticsEngine.js'
...
function analyzeLearning(stats = null) {
  ...
  const weakSubjects   = breakdown.filter(c => weakIds.includes(c.id) || c.accuracy < 60)
  const strongSubjects = breakdown.filter(c => c.accuracy >= 80).sort((a, b) => b.accuracy - a.accuracy)
  const trend          = getWeeklyTrend()

  let summary
  if (breakdown.length === 0) {
    summary = "You haven't taken any quizzes yet — start with a quick session to build your profile."
    ...
    summary = `${weakSubjects.length} subject${weakSubjects.length > 1 ? 's' : ''} need attention: ${weakSubjects.slice(0, 2).map(w => w.label).join(', ')}.`
  }
...
function getPersonalizedGuidance(stats = null) {
  const { weakSubjects, strongSubjects, trend, stats: s } = analyzeLearning(stats)
  const tips = []
...
function getSyllabusRecommendations() {
  try {
    ...
    const needsRevision = subjects
      ...
      .sort((a, b) => (b.progress.revisionNeeded ?? 0) - (a.progress.revisionNeeded ?? 0))
    ...
    const almostDone = subjects
      .filter(s => (s.progress?.pct ?? 0) >= 80 && (s.progress?.pct ?? 0) < 100)
    ...
    if (neglected.length > 0 && started.length > 0) {
      const pick = neglected[Math.floor(Math.random() * Math.min(neglected.length, 3))]
      recs.push({
        agent:       'coach',
