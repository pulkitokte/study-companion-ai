...
function TopicRow({ topicDef, progress, subjectColor, onAction }) {
  ...
  return (
    <div
      className="border-b border-white/[0.04] last:border-0 px-4 py-3.5 transition-colors hover:bg-white/[0.015]"
      style={{
        background:
    ...
    >
      ...
      <div className="flex items-start gap-3">
        {/* Status dot */}
        <div className="mt-0.5 shrink-0 relative w-3 h-3">
          <div
...
export default function TopicPanel({
  ...
  // Static topic definitions from syllabusData
  const topicDefs = useMemo(
    () => getTopics(examId, subject.id),
    [examId, subject.id],
  );

  // Load / refresh progress from service
  ...
  // Per-filter topic counts for tab labels
  ...
  return (
    ...
    >
      {/* ── Header ──────────────────────────────────────────────────────── */}
      <div
        className="shrink-0 px-5 py-4 border-b border-white/[0.07]"
